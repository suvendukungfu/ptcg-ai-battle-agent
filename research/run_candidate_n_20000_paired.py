"""
20,000 Paired Seed-Matched Evaluation: Candidate N vs Candidate M.
Tests Candidate N (Froslass Priority & Non-EX Swarm Policy) vs Candidate M.
"""
...
def run_20000_paired_evaluation(num_pairs: int = 10000, max_workers: int = 8):
    ...
    for g in range(num_pairs):
        ...
        # N
        tasks.append((f"N_{opp_name}", deck_m, opp_deck, seed, a_is_seat_0))
    ...
    n_matching = [r for r in results if r["cand_name"].startswith("N_")]

    m_wins = sum(r["win"] for r in m_matching)
    n_wins = sum(r["win"] for r in n_matching)
    total_evals = num_pairs

    m_wr = m_wins / total_evals * 100.0
    n_wr = n_wins / total_evals * 100.0
    ...
    p_n = n_wins / total_evals
    n_ci_low = max(0.0, (p_n + z**2/(2*total_evals) - z*math.sqrt((p_n*(1-p_n) + z**2/(4*total_evals))/total_evals))/(1 + z**2/total_evals)*100.0)
    n_ci_high = min(100.0, (p_n + z**2/(2*total_evals) + z*math.sqrt((p_n*(1-p_n) + z**2/(4*total_evals))/total_evals))/(1 + z**2/total_evals)*100.0)

    # Matchup Breakdown
    matchup_stats = {}
    for opp_name, _, _ in opponents:
        m_opp = [r for r in m_matching if r["cand_name"] == f"M_{opp_name}"]
        n_opp = [r for r in n_matching if r["cand_name"] == f"N_{opp_name}"]
        m_w = sum(r["win"] for r in m_opp)
        n_w = sum(r["win"] for r in n_opp)
        tot = len(m_opp)
        matchup_stats[opp_name] = {
            "total": tot,
            "m_wr": round(m_w / tot * 100.0, 2) if tot > 0 else 0.0,
            "n_wr": round(n_w / tot * 100.0, 2) if tot > 0 else 0.0,
            "delta": round((n_w - m_w) / tot * 100.0, 2) if tot > 0 else 0.0,
        }
        print(f">> vs {opp_name:20s}: M={matchup_stats[opp_name]['m_wr']:5.1f}% | N={matchup_stats[opp_name]['n_wr']:5.1f}% | Delta={matchup_stats[opp_name]['delta']:+5.1f}%")

    print(f"\n--- 20,000 PAIRED EVALUATION STATISTICAL VERDICT ---")
    print(f"Candidate M Win Rate: {m_wr:.2f}% (95% CI: [{m_ci_low:.2f}% – {m_ci_high:.2f}%])")
    print(f"Candidate N Win Rate: {n_wr:.2f}% (95% CI: [{n_ci_low:.2f}% – {n_ci_high:.2f}%])")
    print(f"Net Delta: {n_wr - m_wr:+.2f}%")

    with open("research/candidate_n_20000_paired.json", "w") as f:
        json.dump({
            ...
            "n_ci_95": [round(n_ci_low, 2), round(n_ci_high, 2)],
            "delta": round(n_wr - m_wr, 2),
