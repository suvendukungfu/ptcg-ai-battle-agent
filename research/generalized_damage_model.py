"""
Generalized Card-Database-Backed Damage and Threat Model.
Derives damage capabilities, conditional scaling, Safeguard interactions,
and lethal probabilities directly from data/EN Card Data.csv.
"""
...
class GeneralizedDamageModel:
    ...
    without hardcoding specific card IDs or names.
    """
    ...
    def has_safeguard_immunity(target: Optional[Dict[str, Any]]) -> bool:
        """
        Derives Safeguard immunity directly from card abilities/skills text.
        """
        ...
        for s in skills:
            if isinstance(s, dict):
                ...
                if "safeguard" in name or ("prevent all damage" in text and "pokémon {ex}" in text):
                    return True
        return False
    ...
    def get_pokemon_profile(card_or_dict: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if is_ex:
            ...
            prize_value = 1
            nominal_cost = 2
            base_damage = 120.0 if hp >= 130 else 80.0
        else:  # Basic
            prize_value = 1
    ...
    def calculate_expected_damage(
        ...
        if energy_count < nominal_cost:
            # Partial charge produces reduced or setup damage
            if energy_count >= 1:
    ...
    @staticmethod
    def calculate_lethal_probability(
        attacker: Optional[Dict[str, Any]],
        target: Optional[Dict[str, Any]],
        target_remaining_hp: float,
        opp_bench_count: int = 0,
        energy_count: int = 0,
    ) -> Tuple[bool, float]:
        """
        Returns (is_lethal, expected_damage).
        """
        dmg = GeneralizedDamageModel.calculate_expected_damage(
            attacker, target, opp_bench_count, energy_count
        )
        is_lethal = dmg >= target_remaining_hp if target_remaining_hp > 0 else False
        return is_lethal, dmg
