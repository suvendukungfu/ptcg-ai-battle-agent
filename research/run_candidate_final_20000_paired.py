"""
20,000 Paired Seed-Matched Evaluation: Candidate M vs Candidate FINAL.
Evaluates Candidate FINAL (Dynamic Ability Threat Denial & Auxiliary Single-Prize Tactical Policy) vs Candidate M.
"""
...
def run_20000_paired_evaluation(num_pairs: int = 10000, max_workers: int = 8):
    ...
    opponents = [
        ("Grimmsnarl_Froslass", load_deck_from_file("research/deck_candidates/OPP_Duraludon_NonEX.csv"), 0.25),
        ...
        ("Trevenant_Bramble", load_deck_from_file("research/deck_candidates/OPP_Cinderace_NonEX.csv"), 0.15),
    ]
    ...
    for g in range(num_pairs):
        ...
        # FINAL
        tasks.append((f"FINAL_{opp_name}", deck_m, opp_deck, seed, a_is_seat_0))

    total_tasks = len(tasks)
    print(f"Executing {total_tasks} paired seed-matched evaluations (Candidate M vs Candidate FINAL) across {max_workers} workers...")
    t0 = time.time()
    ...
    final_matching = [r for r in results if r["cand_name"].startswith("FINAL_")]

    m_wins = sum(r["win"] for r in m_matching)
    final_wins = sum(r["win"] for r in final_matching)
    total_evals = num_pairs

    m_wr = m_wins / total_evals * 100.0
    final_wr = final_wins / total_evals * 100.0
    ...
    p_final = final_wins / total_evals
    final_ci_low = max(0.0, (p_final + z**2/(2*total_evals) - z*math.sqrt((p_final*(1-p_final) + z**2/(4*total_evals))/total_evals))/(1 + z**2/total_evals)*100.0)
    final_ci_high = min(100.0, (p_final + z**2/(2*total_evals) + z*math.sqrt((p_final*(1-p_final) + z**2/(4*total_evals))/total_evals))/(1 + z**2/total_evals)*100.0)

    # Matchup Breakdown
    matchup_stats = {}
    for opp_name, _, _ in opponents:
        m_opp = [r for r in m_matching if r["cand_name"] == f"M_{opp_name}"]
        final_opp = [r for r in final_matching if r["cand_name"] == f"FINAL_{opp_name}"]
        m_w = sum(r["win"] for r in m_opp)
        final_w = sum(r["win"] for r in final_opp)
        tot = len(m_opp)
        matchup_stats[opp_name] = {
            "total": tot,
            "m_wr": round(m_w / tot * 100.0, 2) if tot > 0 else 0.0,
            "final_wr": round(final_w / tot * 100.0, 2) if tot > 0 else 0.0,
            "delta": round((final_w - m_w) / tot * 100.0, 2) if tot > 0 else 0.0,
        }
        print(f">> vs {opp_name:20s}: M={matchup_stats[opp_name]['m_wr']:5.1f}% | FINAL={matchup_stats[opp_name]['final_wr']:5.1f}% | Delta={matchup_stats[opp_name]['delta']:+5.1f}%")

    print(f"\n--- 20,000 PAIRED EVALUATION STATISTICAL VERDICT ---")
    print(f"Candidate M Win Rate: {m_wr:.2f}% (95% CI: [{m_ci_low:.2f}% – {m_ci_high:.2f}%])")
    print(f"Candidate FINAL Win Rate: {final_wr:.2f}% (95% CI: [{final_ci_low:.2f}% – {final_ci_high:.2f}%])")
    print(f"Net Delta: {final_wr - m_wr:+.2f}%")

    with open("research/candidate_final_20000_paired.json", "w") as f:
        json.dump({
            "total_games": total_tasks,
            "m_wins": m_wins,
            "final_wins": final_wins,
            "m_wr": round(m_wr, 2),
            "final_wr": round(final_wr, 2),
            "m_ci_95": [round(m_ci_low, 2), round(m_ci_high, 2)],
            "final_ci_95": [round(final_ci_low, 2), round(final_ci_high, 2)],
            "delta": round(final_wr - m_wr, 2),
            "matchups": matchup_stats,
        }, f, indent=2)

    return m_wr, final_wr
