...
export const ReplayExplorerView: React.FC = () => {
  ...
  return (
    <div className="space-y-6 text-left pb-12">
        ...
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-2xl font-black text-white tracking-tight flex items-center gap-2">
              ...
              Replay Explorer & Momentum Science
            </h2>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Turn-by-turn trajectory scrubber, prize race curve, board momentum differentials, and key turning-point annotations.
          </p>
        </div>

        <div className="flex items-center gap-2">
          <span className="text-xs font-mono text-slate-400">Total Steps: {timeline.length}</span>
          <span className="text-xs font-mono font-bold text-emerald-400 px-3 py-1 rounded-xl bg-emerald-500/10 border border-emerald-500/20">
            Winner: {simulationData?.winner || 'YOU'}
          </span>
        </div>
      </div>

      {/* 2. Prize Momentum Differential Curve (SVG) */}
      <div className="glass-panel p-5 rounded-2xl border border-white/8 space-y-4">
        ...
        <div className="h-44 w-full relative">
          <svg className="w-full h-full overflow-visible" preserveAspectRatio="none" viewBox="0 0 800 160">
            ...
            {/* Current Step Scrubber Indicator */}
            {timeline.length > 0 && (
              <line
        ...
        <div className="flex items-center gap-3 pt-2 border-t border-white/6">
          <button
            ...
            className="p-2 rounded-lg bg-white/4 hover:bg-white/10 text-slate-300 transition-colors"
            title="Jump to Start"
          >
          ...
          <button
            ...
            className="px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs flex items-center gap-1.5 shadow-md shadow-indigo-600/20 transition-all"
          >
      ...
      {/* 3. Replay Split: Current Turn Snapshot & Event Log */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        {/* Left 6 Cols: Step Snapshot Inspector */}
        <div className="lg:col-span-6 glass-panel p-5 rounded-2xl border border-white/8 space-y-4">
          <div className="flex items-center justify-between pb-2 border-b border-white/8">
            <span className="text-xs font-bold text-white uppercase tracking-wider font-mono flex items-center gap-1.5">
              ...
              Snapshot State (Turn {stepData.turn || 1})
            </span>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="p-3.5 rounded-xl bg-indigo-950/30 border border-indigo-500/20 space-y-1.5">
              <div className="flex justify-between items-center text-xs">
                <span className="font-bold text-indigo-200">Your Active</span>
                <span className="text-[10px] font-mono text-emerald-400">
                  HP {stepData.your_active?.hp || 350}/{stepData.your_active?.maxHp || 350}
                </span>
              </div>
              <div className="text-sm font-black text-white">
                {stepData.your_active?.id === 723 ? 'Bellibolt ex' : `Pokemon #${stepData.your_active?.id || 721}`}
              </div>
              <div className="text-[10px] font-mono text-slate-400 flex items-center gap-1">
                <Zap className="w-3 h-3 text-amber-400" />
                Attached: {stepData.your_active?.energies?.length || 2}⚡ Lightning
              </div>
            </div>

            <div className="p-3.5 rounded-xl bg-rose-950/30 border border-rose-500/20 space-y-1.5">
              <div className="flex justify-between items-center text-xs">
                <span className="font-bold text-rose-200">Opponent Active</span>
                <span className="text-[10px] font-mono text-rose-400">
                  HP {stepData.opp_active?.hp || 150}/{stepData.opp_active?.maxHp || 150}
                </span>
              </div>
              <div className="text-sm font-black text-white">
                {stepData.opp_active?.id === 721 ? 'Tadbulb' : stepData.opp_active?.id === 345 ? 'Crustle' : `Pokemon #${stepData.opp_active?.id || 721}`}
              </div>
              <div className="text-[10px] font-mono text-slate-400 flex items-center gap-1">
                <Zap className="w-3 h-3 text-amber-400" />
                Attached: {stepData.opp_active?.energies?.length || 0}⚡
              </div>
            </div>
          </div>

          {/* Action Taken at this step */}
          <div className="p-3 rounded-xl bg-white/2 border border-white/6 text-xs font-mono space-y-1">
            <div className="text-slate-400 text-[10px] uppercase tracking-wider">Action Executed</div>
              ...
              {['all', 'attack', 'energy', 'trainer'].map((f) => (
                <button
                  ...
                  className={`px-2 py-0.5 rounded capitalize font-bold transition-all ${
                    selectedFilter === f ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-slate-200'
                  }`}
                >
                  {f}
                </button>
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-center text-[10px] text-slate-400 mb-0.5">
                    <span className="font-bold text-indigo-300">Turn {ev.turn || 1} • Step {idx + 1}</span>
