...
class GoalPlanner:
    ...
    Identifies high-level macro objectives based on prize counts, board state,
    ...
    def identify_goal(state: GameState) -> GoalState:
        ...
        # 5. Build Bench & Prepare Attacker (Early Game Setup)
        ...
        if len(your_bench) < 2 or (your_active and len(active_energies) < 2):
            return GoalState(
                primary_goal=StrategicGoal.GOAL_PREPARE_ATTACKER,
                secondary_goal=StrategicGoal.GOAL_BUILD_BENCH,
                goal_rationale="Early game tempo development. Benching Tadbulb and accelerating energy with Electric Generator.",
                attack_priority_bonus=1200.0,
                energy_priority_bonus=3000.0,
                evolution_priority_bonus=3500.0,
                gust_priority_bonus=800.0,
            )

        # 6. Default Two-Prize KO Hunt
        return GoalState(
            primary_goal=StrategicGoal.GOAL_TAKE_TWO_PRIZE_KO,
            secondary_goal=StrategicGoal.GOAL_TEMPO_DEVELOPMENT,
            goal_rationale="Mid-game aggressive board progression. Executing maximum damage Electro Bullet turns.",
            attack_priority_bonus=2500.0,
            energy_priority_bonus=2000.0,
            evolution_priority_bonus=2000.0,
            gust_priority_bonus=2200.0,
        )
